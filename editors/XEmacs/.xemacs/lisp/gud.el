...
;;				 ^^^^^^^^^
;; [This should encourage gud extensions that invoke gdb commands to let
...
It must return output (if any) to be inserted in the gud 
buffer."
...
;      ["Duel examples"		(gdb-help "duel examples") t]
  ...
  (if (and (featurep 'menubar)
	   current-menubar (not (assoc "Gdb" current-menubar)))
      (progn
...
(defun gud-load (instance command)
  (interactive
   (let ((temp-buffer-show-function (function gud-temp-buffer-show)))
     (list (gdb-needed-default-instance)
	   (concat "load " (read-file-name "Dynamically load from file: "
					   nil
					   "a.out"
					   t)))))
  (gud-display-gud-buffer instance)
