...
;; Inserted by crpi
(defcustom default-login-name (concat user-login-name "@nl.imc.local")
"The default name to be used when logging into confluence"
  :group 'confluence
  :type 'string)

;; Inserted by crpi
(defcustom default-confluence-password nil
  "Default password to be used when logging into confluence"
  :group 'confluence
  :type 'string)
        ...
        (condition-case err
                    ...
                    (cf-rpc-execute-internal 
                     ...
                     (setq username
                           (or (car-safe credentials)
                               (read-string (format "Confluence Username [%s]: " default-login-name)
                                            nil nil default-login-name t)))
                     ;; Changed by crpi: added default password
                     (setq password
                           (or (cdr-safe credentials)
                               (read-passwd "Confluence Password: " nil default-confluence-password)))))
              (cf-set-struct-value 'confluence-login-token-alist
          ...
          (error
           (progn
             ...
             (message "Default password:" default-confluence-password)
...
M-* `confluence-pop-tag-stack')."
  ...
  (if (eq space-name nil)
      (setq space-name (cf-get-default-space)))

  (cf-prompt-page-info nil 'page-name 'space-name)
  (setq page-name (replace-regexp-in-string "+" " " (file-name-nondirectory page-name)))
